...
namespace PersonalFinances.Forms
{
    public partial class EditTransaction : Form
    {
        ...
        {
            var dossierDetail = dOSSIER_DETAILSTableAdapter.GetData().FirstOrDefault(x=> x.DD_ID == -1);
            var transactionNo = dossierDetailsIdComboBox.Text;

            currentTransactionNo = transactionNo;
            if (!string.IsNullOrEmpty(transactionNo) && transactionNo.All(char.IsDigit))
            {
               dossierDetail = dOSSIER_DETAILSTableAdapter
                .GetData().FirstOrDefault(x => x.DD_ID == Decimal.Parse(transactionNo) && x.DOSSIER_NO == Decimal.Parse(dossierId));
        ...
        {
            dossierDetailsIdComboBox.Items.Clear();
            FillTransactionNo(dOSSIER_DETAILSTableAdapter, dossierDetailsIdComboBox);
